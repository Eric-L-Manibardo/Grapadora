...
draw all flow recordings during a year
"""
import pandas as pd
import numpy as np
...
#AX1
i=1   #selecciona espira
df_flow = pd.read_csv('C-colombia_BATCH/'+str(ATR[i])+'_B.csv',index_col=0)
selected_days, selected_weekends = compute_split(df_flow)

# dibujamos la +-std de cada uno de las muestras disponibles para cada timestamp 
sns.lineplot(data=compute_dfdays(selected_days),x='time',y='flow',estimator=np.median,ci='sd',ax=ax1, legend='auto')    
sns.lineplot(data=compute_dfdays(selected_weekends),x='time',y='flow',estimator=np.median,ci='sd',ax=ax1)

#AX2
i=45 #selecciona espira
df_flow = pd.read_csv('C-colombia_BATCH/'+str(ATR[i])+'_B.csv',index_col=0)
selected_days, selected_weekends = compute_split(df_flow)

# dibujamos la +-std de cada uno de las muestras disponibles para cada timestamp 
sns.lineplot(data=compute_dfdays(selected_days),x='time',y='flow',estimator=np.median,ci='sd',ax=ax2)    
sns.lineplot(data=compute_dfdays(selected_weekends),x='time',y='flow',estimator=np.median,ci='sd',ax=ax2)

#AX3
i=0  #selecciona espira
df_flow = pd.read_csv('C-colombia_BATCH/'+str(ATR[i])+'_B.csv',index_col=0)
selected_days, selected_weekends = compute_split(df_flow)

# dibujamos la +-std de cada uno de las muestras disponibles para cada timestamp 
sns.lineplot(data=compute_dfdays(selected_days),x='time',y='flow',estimator=np.median,ci='sd',ax=ax3)    
sns.lineplot(data=compute_dfdays(selected_weekends),x='time',y='flow',estimator=np.median,ci='sd',ax=ax3)
...
ax2.title.set_text('ATR 45')
ax3.title.set_text('ATR 1')
